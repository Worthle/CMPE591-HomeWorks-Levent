...
def train():
    ...
    for episode in range(episodes):
        ...
        if episode % 1000 == 0:
            ...
            torch.save(agent.actor.state_dict(), f"actor_sac2_{episode}.pth")
...
def test():
    ...
    agent = SACAgent(state_dim=obs_dim, action_dim=act_dim, lr=learning_rate, eta_min= 5e-4)
    agent.actor.load_state_dict(torch.load("actor_sac_600.pth"))
    agent.actor.eval()
