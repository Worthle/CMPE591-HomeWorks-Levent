...
class Hw3Env(environment.BaseEnv):
    def __init__(self, **kwargs) -> None:
        ...
        self._max_timesteps = 300  # allow more steps 300
    ...
    def _create_scene(self, seed=None):
        ...
        environment.create_visual(scene, "cylinder", pos=goal_pos, quat=[0, 0, 0, 1],
                                  ...
                                  name="goal")
        #environment.create_ur5e_robotiq85f()
        return scene
    
    def reset(self):
        super().reset()
        self._prev_obj_pos = self.data.body("obj1").xpos[:2].copy()  # initialize previous position
        self._prev_ee_pos = self.data.site(self._ee_site).xpos[:2].copy()  # initialize previous position
        self._t = 0
    ...
    def reward(self):
        ...
        r_ee_to_obj = -0.02 * d_ee_to_obj  # getting closer to object
        r_obj_to_goal = -0.05 * d_obj_to_goal  # moving object to goal


        # direction bonus
        obj_movement = obj_pos - self._prev_obj_pos
        ee_movement = ee_pos - self._prev_ee_pos

        # no movement penalty
        if np.linalg.norm(ee_movement) > 1e-6:
            r_stuck = 0.0
        else:
            if self.is_terminal():
                r_stuck = -1.0 + 10.0
        ...
        ee_dir_to_goal = (goal_pos - ee_pos) / (np.linalg.norm(goal_pos - ee_pos) + 1e-8)

        ee_to_obj = (np.linalg.norm(obj_pos - ee_pos) + 1e-8)
        ...
        norm_ee_to_obj = ee_to_obj / 0.423 # assuming the maximum distance is 300*sqrt(2) since grid size is 300x300
        
        #r_direction_obj = 0.5 * max(0, np.dot(obj_movement / (np.linalg.norm(obj_movement) + 1e-8), dir_to_goal))#0.5
        r_direction_obj = (1 - norm_ee_to_obj) * max(0, np.dot(obj_movement / (np.linalg.norm(obj_movement) + 1e-8), dir_to_goal))#0.5
        r_direction_ee = (norm_ee_to_obj) * max(0, np.dot(ee_movement / (np.linalg.norm(ee_movement) + 1e-8), ee_dir_to_goal))#0.5
        r_direction = r_direction_obj + r_direction_ee
        
        if np.linalg.norm(obj_movement) + np.linalg.norm(ee_movement) < 1e-6:  # Avoid division by zero
            r_direction = 0.0

        # terminal bonus
        r_terminal = 10.0 if self.is_terminal() else 0.0

        r_step = -0.0001  # penalty for each step

        self._prev_obj_pos = obj_pos.copy()
        self._prev_ee_pos = ee_pos.copy()
        if r_stuck == 9.0 or r_stuck == -1.0:
            print(r_stuck)
        return r_ee_to_obj + r_obj_to_goal + r_direction + r_terminal + r_step + r_stuck + r_close
