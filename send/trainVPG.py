...
if __name__ == "__main__":
    ...
    for episode in range(num_episodes):
        ...
        if episode % 100 == 0:
            ...
            np.save(f"VPG/rewardsVPG{7*num_episodes+9030}_{episode}.npy", np.array(rewards))
            np.save(f"VPG/RPSVPG{7*num_episodes+9030}_{episode}.npy", np.array(RPS))
        if episode % 100 == 0:
            plt.close()
            plt.subplot(1,2,1)
    ...
    agent.save_model(10000,7*num_episodes+9010)
    np.save(f"VPG/rewardsVPG{7*num_episodes+9010}_{10000}.npy", np.array(rewards))
    np.save(f"VPG/RPSVPG{7*num_episodes+9010}_{10000}.npy", np.array(RPS))