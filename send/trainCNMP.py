...
def generate_demonstrations():
    ...
    env._set_ee_in_cartesian(curve[0], rotation=[-90, 0, 180], n_splits=100, max_iters=t_steps, threshold=0.05)
    states = []
    for p in curve:
...
X,Y = (np.load("CNMP_Train_X_10.npy"),np.load("CNMP_Train_Y_10.npy"))
v_X, v_Y = (np.load('CNMP_Train_X_10.npy'), np.load('CNMP_Train_X_10.npy'))
d_x , d_y = (X.shape[-1] , Y.shape[-1])
d_N = X.shape[0]
obs_max = 3
time_len = X.shape[1]
